...
What's working so far (I'm up to v 0.5-2 by now):
...
* File Manager - native (double click works, navigation, file and folder creation work.  search not tested)
* Custom Icons for FM (configurable via text file)
* Automounter - native (runsnon start from session file- works)
* Screenshot utility - native (command only, no gui - full screen and area select)
...
* media player that doesn't load half of QT or half of GTK
